...
# 从文件中获取参数
def get_cfg(args):
    file_path = 'configs/'+args.dataset+'/' + args.model + '/cfg.json'
...
def get_model_training_params(cfg):
    model_param_spaces = {
        "DecisionTransformer": cfg['model']
    }
    training_args = {
        'DecisionTransformer': cfg['training']
    }
...
def trainer(cfg):
    # 模型参数准备
    cfg_model, cfg_training = get_model_training_params(cfg)
    print(cfg_training)
    ...
    if args.model in ['DecisionTransformer']:
        ...
        optimizer = make_optimizer(
            cfg_training[args.model]['optimizer'],
            (
                [
            ...
            cfg_training[args.model]['lr'],
            cfg_training[args.model]['weight_decay'],
        )
    ...
    # 训练开始
    test_score = train(model, optimizer)
    save_path = args.save_path
    save_path = save_path + args.dataset + '/'
    create_folder(save_path)
    save_file = save_path + args.model + '.pt'
